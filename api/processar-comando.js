...
function interpretarDataHoraComDayjs(dataRelativa, horarioTexto) {
  ...
  console.log(`interpretarDataHora (v30): Input: dataRelativa='${dataRelativa}', horarioTexto='${horarioTexto}'`);
  console.log(`interpretarDataHora (v30): Agora em São Paulo (referência): ${agoraEmSaoPaulo.format('YYYY-MM-DD HH:mm:ss Z')}`);
  console.log(`interpretarDataHora (v30): Normalizado: dataNorm='${dataNorm}', horarioProcessado='${horarioProcessado}'`);
  ...
  if (dataNorm === "hoje") { 
    ...
    if (diasDaSemanaMap[nomeDiaParaBusca] !== undefined) {
        ...
        if (ehProximaSemana) {
            if (diff === 0) { // Se é o mesmo dia da semana e pediu "próxima"
                diff = 7;
            }
            // Se não é o mesmo dia, e pediu "próxima", a lógica de diff já aponta para a próxima semana
            // ou para um dia futuro nesta semana. Se for futuro nesta semana, e pediu "próxima", adiciona 7.
            // A lógica do PDF que funcionou no seu testador era: if (diff === 0) diff = 7;
            // E depois dataAlvo = agora.add(diff, 'day').startOf('day');
            // Isto significa que "próxima sexta" a partir de uma segunda, resultaria em diff = 4 (sexta desta semana).
            // Se a intenção de "próxima" é sempre a semana seguinte, mesmo que o dia ainda não tenha chegado nesta semana:
            // if (diff !== 0) { // Se não é o mesmo dia da semana
            //     diff += 7; // Força para a semana seguinte
            // }
            // No entanto, o seu teste bem-sucedido mostrou que "próxima sexta" (hoje segunda) deu a sexta desta semana.
            // Então, a lógica do PDF ( (alvo - hoje + 7) % 7 || 7 ) se for "próxima" e mesmo dia, senão só (alvo-hoje+7)%7
            // Vamos usar a lógica que funcionou no seu testador:
            // se 'próxima' é true E diff é 0, então diff = 7. Senão, o diff calculado é usado.
        }
        
        dataAlvo = agoraEmSaoPaulo.add(diff, 'day').startOf('day');
        console.log(`interpretarDataHora (v30): Dia da semana '${dataRelativa}' (ehProxima: ${ehProximaSemana}, hojeNum: ${hojeNum}, diaAlvoNum: ${diaAlvoNum}, diff: ${diff}) interpretado como:`, dataAlvo.format('YYYY-MM-DD'));
        ...
        if (matchMesExtenso) {
            ...
            if (dia && mes) {
                ...
                if(dataParseada.isValid()) console.log(`interpretarDataHora (v30): Data por extenso parseada:`, dataParseada.format('YYYY-MM-DD'));
            }
        }
        ...
        if (!dataParseada || !dataParseada.isValid()) { 
            ...
            for (const formato of formatosData) {
              ...
              if (dataParseada.isValid()) {
                console.log(`interpretarDataHora (v30): Data parseada com formato '${formato}':`, dataParseada.format('YYYY-MM-DD'));
                break; 
              }
            }
        }
        ...
        if (dataParseada && dataParseada.isValid()) {
          ...
          console.error("interpretarDataHora (v30): Formato de data não reconhecido:", dataRelativa);
          return null;
        }
    }
  }
  ...
  if (horarioProcessado === "meio-dia") { 
    ...
    if (matchHorario) {
      ...
      if (horas < 0 || horas > 23 || minutos < 0 || minutos > 59) {
          console.error("interpretarDataHora (v30): Horas/minutos inválidos", {horas, minutos});
          return null;
      }
    } else { 
      console.error("interpretarDataHora (v30): Formato de horário não reconhecido", horarioProcessado);
      return null; 
    }
  }
  ...
  if (!dataHoraFinalEmSaoPaulo.isValid()) {
      console.error("interpretarDataHora (v30): Data/Hora final inválida em SP", dataHoraFinalEmSaoPaulo);
      return null;
  }
  console.log("interpretarDataHora (v30): Data/Hora final em São Paulo:", dataHoraFinalEmSaoPaulo.format('YYYY-MM-DD HH:mm:ss Z'));
...
async function gerarRespostaConversacional(contextoParaIA) {
  ...
  console.log("Backend (v30): Gerando resposta conversacional com contexto:", contextoParaIA);
  try {
    ...
    console.error("Backend (v30): Erro ao gerar resposta conversacional com OpenAI:", error);
...
export default async function handler(req, res) {
  // BACKEND V30 - Log de início
  console.log("Backend (v30): Função handler iniciada.");
  ...
  console.log("Backend (v30): Comando recebido:", comando);
  ...
  try {
    ...
    `;
    console.log("Backend (v30): Enviando para OpenAI para extração...");
    ...
    let dadosComando;
    try {
      console.log("Backend (v30): Resposta JSON BRUTA da OpenAI:", extracaoOpenAI.choices[0].message.content);
      dadosComando = JSON.parse(extracaoOpenAI.choices[0].message.content);
    } catch (e) {
      console.error("Backend (v30): Erro parse JSON da extração OpenAI:", e, extracaoOpenAI.choices[0].message.content);
    ...
    console.log("Backend (v30): Dados extraídos pela OpenAI:", dadosComando);

    // ETAPA 2: Lógica de Negócio e Supabase
    if (dadosComando.mensagem_clarificacao_necessaria) {
      console.log("Backend (v30): Clarificação necessária:", dadosComando.mensagem_clarificacao_necessaria);
      ...
      switch (dadosComando.intencao) {
        ...
        case "cancelar_reuniao":
          console.log("Backend (v30): Intenção de cancelar reunião detectada.");
          ...
          if (!idParaCancelar && dadosComando.pessoa_alvo && dadosComando.data_alvo && dadosComando.horario_alvo) {
            console.log("Backend (v30): Tentando encontrar para cancelar por descrição:", dadosComando.pessoa_alvo, dadosComando.data_alvo, dadosComando.horario_alvo);
        ...
        case "alterar_reuniao":
            ...
            if (erroUpdate || !updateData) {
                 console.error("Backend (v29): Erro ao alterar ou compromisso não encontrado:", erroUpdate);
    ...
    console.error("Backend (v29): Erro geral no processamento do comando:", error);
